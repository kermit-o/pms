...
import { ApiError, getRevenueReport, type RevenueReport } from '@/lib/api';
...
export default async function RevenueReportPage({ searchParams }: PageProps) {
  ...
  if (propertyId) {
    try {
      report = await getRevenueReport(session?.accessToken, propertyId, from, to);
    } catch (err) {
  ...
  return (
    <main className="mx-auto max-w-4xl space-y-6 px-6 py-10">
      ...
      <header>
        ...
        <h1 className="text-3xl font-semibold text-aubergine-700">Revenue</h1>
