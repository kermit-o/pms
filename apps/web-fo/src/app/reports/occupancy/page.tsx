...
import { ApiError, getOccupancyReport, type OccupancyReport } from '@/lib/api';
...
export default async function OccupancyReportPage({ searchParams }: PageProps) {
  ...
  if (propertyId) {
    try {
      report = await getOccupancyReport(session?.accessToken, propertyId, from, to);
    } catch (err) {
  ...
  return (
    <main className="mx-auto max-w-4xl space-y-6 px-6 py-10">
      ...
      <header>
        ...
        <h1 className="text-3xl font-semibold text-aubergine-700">Occupancy</h1>
          ...
          {report && (
            <>
              ...
              <strong className="text-aubergine-700">
                {(report.averageOccupancyPct * 100).toFixed(1)}%
              </strong>
            </>
          )}
        </p>
      ...
      >
        ...
        {propertyId && (
          <a
            href={`/api/reports/occupancy?propertyId=${propertyId}&from=${from}&to=${to}`}
          ...
          <table className="w-full text-sm">
            <thead className="bg-aubergine-50 text-left text-xs uppercase tracking-wide text-aubergine-500">
              <tr>
                ...
                <th className="px-4 py-2 text-right">Habitaciones</th>
                <th className="px-4 py-2 text-right">Ocupadas</th>
            ...
            <tbody className="divide-y divide-aubergine-100/70">
              ...
              {report.rows.map((r) => (
                <tr key={r.businessDate}>
                  ...
                  <td className="px-4 py-2 text-right text-aubergine-700/70">{r.totalRooms}</td>
                  <td className="px-4 py-2 text-right text-aubergine-700/70">{r.occupied}</td>
                  <td className="px-4 py-2 text-right font-medium">
                    {(r.occupancyPct * 100).toFixed(1)}%
                  </td>
                </tr>
              ))}
            </tbody>
