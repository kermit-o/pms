...
/**
 * Sprint 12 W3 — pre-pago full PaymentIntent on-session desde el IBE.
 * Espejo de `StripeCardCapture` (SetupIntent) pero con `confirmPayment`
 * en lugar de `confirmSetup`. Tras éxito el server marca SECURED y
 * postea el cobro al folio idempotentemente.
 */
export function StripeChargePayment({
  slug,
  code,
  lastName,
  lang,
  ...
  async function start() {
    ...
    try {
      const res = await fetch(
        `/api/payment-intent?slug=${encodeURIComponent(slug)}&code=${encodeURIComponent(code)}&lastName=${encodeURIComponent(lastName)}`,
        { method: 'POST' },
      );
      if (!res.ok) {
  ...
  return (
    <>
      <button
        ...
        onClick={() => void start()}
        disabled={busy}
        className="rounded-xl bg-amber-700 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-amber-800 disabled:opacity-50"
      >
        {busy
          ? lang === 'es'
            ? 'Cargando…'
      ...
      {open && clientSecret && stripePromise && paymentIntentId && (
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (paymentIntent?.status === 'succeeded') {
        const res = await fetch(
          `/api/confirm-payment-intent?slug=${encodeURIComponent(slug)}&code=${encodeURIComponent(code)}&lastName=${encodeURIComponent(lastName)}`,
          {
            method: 'POST',
  ...
  return (
    <form onSubmit={submit} className="mt-4 space-y-4">
      ...
      <div className="flex justify-end gap-2">
        ...
        >
          {busy ? (lang === 'es' ? 'Procesando…' : 'Processing…') : lang === 'es' ? 'Pagar' : 'Pay'}
        </button>
      </div>
    </form>
  );
